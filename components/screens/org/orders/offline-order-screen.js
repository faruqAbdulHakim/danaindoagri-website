...
export default function OfflineOrderScreen({ isMarketing }) {
  ...
  const loadMoreData = () => {
    setLoadMore(true);
    OrderFetcher.fetchOrders(page, '', 'offline')
  ...
  useEffect(() => {
    setFetching(true);
    OrderFetcher.fetchOrders(1, '', 'offline')
      .then(({ data, error, route }) => {
  ...
  return (
    <>
      <div className="p-6 bg-white/80 h-full backdrop-blur-md overflow-hidden">
        <h1 className="text-xl font-semibold">Data Pemesanan (Offline)</h1>
        ...
        <div
          className={`mt-4 overflow-auto border rounded-lg p-4 bg-white/40 shadow-md
                ...
                return (
                  ...
                  >
                    ...
                    <div className="overflow-clip w-20">
                      <p className="text-xs text-slate-400">Kuantitas</p>
                      <p>{qty}</p>
                    </div>
                    <div className="overflow-clip w-40">
                      <p className="text-xs text-slate-400">Total Harga</p>
                      <p>Rp {productPrice + shipmentPrice + codePrice}</p>
                    </div>
                    <div className="w-52">
                      <div className="flex items-center gap-2">
                        <IoWallet
                          size={20}
                          className={`${
                            status === 'belum dibayar' && 'text-primary'
                          }`}
                        />
                        <IoCheckmarkCircle
                          size={20}
                          className={`${
                            status === 'dikonfirmasi' && 'text-primary'
                          }`}
                        />
                        <FaTruck
                          size={20}
                          className={`${
                            status === 'dikirim' && 'text-primary'
                          }`}
                        />
                        <IoStar
                          size={20}
                          className={`${
                            status === 'diterima' && 'text-primary'
                          }`}
                        />
                      </div>
                    </div>
                    <div className="overflow-clip w-40 flex items-center gap-4">
                      <Link href={`/org/orders/offline/${Order.id}`}>
                        <a className="hover:text-primary">Detail</a>
                      </Link>
                      <Link href={`/org/orders/offline/${Order.id}/edit`}>
                        <a
                          className="bg-primary text-white px-4 py-2 rounded-full
                          hover:opacity-70 active:opacity-40 transition-all inline-block"
                        >
                          Ubah
                        </a>
                      </Link>
      ...
      {error && <CommonErrorModal text={error} onClick={() => setError('')} />}
    </>
  );
}

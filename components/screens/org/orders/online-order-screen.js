...
export default function OnlineOrderScreen() {
  ...
  const searchHandler = (event) => {
    ...
    OrderFetcher.fetchOrders(1, searchText, 'online')
  ...
  const loadMoreData = () => {
    setLoadMore(true);
    const searchText = searchInputRef.current.value;
    OrderFetcher.fetchOrders(page, searchText, 'online')
      .then(({ data, error, route }) => {
        if (data) {
  ...
  useEffect(() => {
    setFetching(true);
    OrderFetcher.fetchOrders(1, '', 'online')
      .then(({ data, error, route }) => {
        if (data) setOnlineOrder(data);
        else if (error) setError(error);
        else if (route) Router.push(route);
  ...
  return (
    <>
      <div className="p-6 bg-white/80 h-full backdrop-blur-md overflow-hidden">
        <h1 className="text-xl font-semibold">Data Pemesanan (Online)</h1>
          ...
          {fetching ? (
            ...
            <>
              {onlineOrder.map((Order) => {
                ...
                rounded-md bg-white/30 mb-3 transition-all"
                  >
                    <div className="overflow-clip w-32">
                      <p className="text-xs text-slate-400">Nama Customer</p>
                    ...
                    <div className="w-20 overflow-clip">
                      <Link href={`/org/orders/online/${Order.id}`}>

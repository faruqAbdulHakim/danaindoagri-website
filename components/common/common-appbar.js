    ...
    <>
      ...
      <div className="hidden lg:inline">
          ...
          <Link href="/org/dashboard">
            <a className="px-4 py-2 text-white hover:opacity-40 active:opacity-70 flex items-center gap-2 transition-all">
              {imageError ? (
                ...
                <Image
                  src={`${BUCKETS.AVATARS.AVATAR_BASE_URL}/avatar${User.id}`}
                  alt=""
                  height={30}
                  width={30}
        ...
        hover:bg-primary hover:border-primary active:opacity-50 flex items-center transition-all"
            ...
            <Link href="/login">
              <a
                className="px-4 py-2 text-white border-l
        hover:bg-primary hover:border-primary active:opacity-50 flex items-center transition-all"
              >
                Masuk
              </a>
...
function NavLink({ text, urlPath, idx }) {
  ...
  return (
    <>
      ...
      >
        <Link href={urlPath}>
          <a className="hover:underline active:opacity-50">{text}</a>
        </Link>
      </motion.li>
    </>
  );
...
function MobileNav({ navLinkList, User }) {
  ...
  return (
    <>
      <button
        className="lg:hidden ml-auto border border-white p-1 rounded-md"
